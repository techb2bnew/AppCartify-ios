...
const AccountDetails = ({ navigation }: { navigation: any }) => {
  const [fullName, setFullName] = useState('TestUser');
  const [email, setEmail] = useState('testuser@appcartify.com');
  const [dateOfBirth, setDateOfBirth] = useState(new Date());
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [gender, setGender] = useState('');
  const [phoneNumber, setPhoneNumber] = useState('9234567890');
  ...
  const handleSubmit = async (id) => {
    logEvent('Submit button clicked in Account details');
    try {
        ...
        await AsyncStorage.setItem('mockUserDetails', JSON.stringify(mockUserDetails));
        navigation.goBack();
        logEvent('Updated customer profile Successfully');
    } catch (error) {
