...
const RegisterScreen = ({ navigation }: { navigation: any }) => {
  ...
  const [phone, setPhone] = useState('9234567890');
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const mockUser = {
        firstName: 'Test',
        lastName: 'User',
        email: 'testuser@appcartify.com',
        phone: '+911234567890',
        password: 'password123',
      };

      // Save mock data to AsyncStorage
      await AsyncStorage.setItem('userDetails', JSON.stringify(mockUser));
      Toast.show(`User Registered Successfully`);
      setSuccessModalVisible(false);
      navigation.navigate('Home');
      dispatch(loginSuccess({ email: mockUser.email, password: mockUser.password }));
      logEvent('User Registered Successfully');
      handleNotificationTrigger();
    } catch (error) {
      console.log('Registration error:', error);
        ...
        {showOTP ?
          ...
          <>
            ...
            <View style={[styles.textInputBox]}>
              ...
              <View style={{ position: "absolute", bottom:hp(15),width:"100%" }}>
